...
if ($nb) {
	if ($pricetowithdraw) {
		...
		$default_account = ($type == 'bank-transfer' ? 'PAYMENTBYBANKTRANSFER_ID_BANKACCOUNT' : 'PRELEVEMENT_ID_BANKACCOUNT');	// InfraS add
		print $form->select_comptes($conf->global->$default_account, 'id_bankaccount', 0, "courant=1", 0, '', 0, '', 1);	// InfraS change
		print ' - ';
...
$sql = "SELECT f.ref, f.rowid, f.date_lim_reglement as datelimite, f.total_ttc,";
$sql .= " s.nom as name, s.rowid as socid,";
$sql .= " pfd.rowid as request_row_id, pfd.date_demande, pfd.amount, pfd.fk_soc_rib";	// InfraS add
if ($type == 'bank-transfer') {
